...
public class JsonFileFormatStrategy : IFileFormatStrategy
{
    ...
    {
        if (string.IsNullOrEmpty(rootElementPath))
        {
            throw new ArgumentException("Root webElement path must be provided for processing JSON objects.", nameof(rootElementPath));
        }

        if (!objectElement.TryGetProperty(rootElementPath, out JsonElement nestedElement))
        {
            throw new KeyNotFoundException($"Root webElement '{rootElementPath}' not found in the JSON.");
        }

        if (nestedElement.ValueKind != JsonValueKind.Array)
        {
            throw new InvalidOperationException($"The specified root webElement '{rootElementPath}' != an array.");
        }
    ...
    private IEnumerable<RelationshipData> ProcessArray(JsonElement arrayElement)
    {
        foreach (JsonElement childItem in arrayElement.EnumerateArray())
        {
            ...
            {
                RelationshipData? data = JsonSerializer.Deserialize<RelationshipData>(childItem.GetRawText(), _serializerOptions);
                if (data != null)
                {
                    yield return data;
                }
