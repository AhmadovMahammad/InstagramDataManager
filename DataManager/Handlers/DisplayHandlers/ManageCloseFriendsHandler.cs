...
public class ManageCloseFriendsHandler() : BaseCommandHandler
{
    ...
    private void ManageData(IEnumerable<RelationshipData> data, IWebDriver webDriver)
    {
        ...
        BuildRemoveFromCloseFriendsTask(taskBuilder, data).ExecuteTasks();
    }

    private ITaskBuilder BuildRemoveFromCloseFriendsTask(SeleniumTaskBuilder taskBuilder, IEnumerable<RelationshipData> data)
    {
        foreach (var relationship in data)
        {
            taskBuilder.PerformAction(driver => HandleCloseFriendProfile(driver, relationship));
        }

        return taskBuilder;
    }

    private void HandleCloseFriendProfile(IWebDriver webDriver, RelationshipData relationship)
    {
        ...
        try
        {
            ...
            var removeButton = FindRemoveFromCloseFriendsButton(webDriver);
            if (removeButton != null)
            {
                removeButton.Click();
                $"Profile '{relationship.Title}' has been successfully removed from close friends.".WriteMessage(MessageType.Success);
            }
            else
            {
                $"No 'Remove from Close Friends' button found for profile '{relationship.Title}'.".WriteMessage(MessageType.Warning);
            }
        }
        catch (Exception ex)
        {
    ...
    private IWebElement? FindRemoveFromCloseFriendsButton(IWebDriver webDriver)
    {
        return null;
    }
}
